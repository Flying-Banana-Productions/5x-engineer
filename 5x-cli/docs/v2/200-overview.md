...
- **Pending-prompt / decision queue.** When `5x prompt` is invoked, it writes a pending row and polls for an answer. The answer may arrive from the terminal **or** from the control plane (#2) — first writer wins. This converts the existing blocking-CLI prompt contract into a two-way channel **without** inter-process signaling or a daemon-to-agent socket. The queue is implemented locally via `PromptStore` (`src/control-plane/`; SQLite is the v2 materialization).
- **Ambient run context.** `5x run init` writes a local control-plane focus pointer (`.5x/current-run`), while commands in linked worktrees first infer a unique active run from the existing plan/worktree mapping. Explicit `--run` and session-scoped `FIVEX_RUN` remain authoritative; the pointer is operator convenience, not invocation identity. Prompt-queue and invocation-registry workers must be given an explicit `run_id` and must not call this ambient resolver.

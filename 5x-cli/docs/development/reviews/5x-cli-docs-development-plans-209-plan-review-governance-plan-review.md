...
## Addendum 6 (2026-09-22) — Re-review of plan v1.6 (concrete plan-208 reconciliation)

**Reviewed:** `docs/development/plans/209-plan-review-governance-plan.md` v1.6 @ `40034b6` (prior reviewed revision `51c20b2`, review commit `1e83dd4`). This review runs under a new run (`run_6614fbffb173`) with a freshly captured baseline (`B0 = 47`, `captureKind: initial`, pinned-by-config mode `advisory`), so it is the first budget-active review for this run and carries an independent baseline assessment below. The prompt's appended diff and commit range were empty again, so I recomputed the delta with `git diff 51c20b2 40034b6 -- <plan>` and re-read Phase 0, §2.1, §2.3, §5.2, §6.1–6.3, Files Touched, and the Tests table.
**Local verification:** Not run (static review). Every concrete plan-208 claim v1.6 introduces was checked against the inspected worktree `.5x/worktrees/208-review-budget-advisory-plan-2f60d7` @ `cd7ee88` (which contains sealed `b3eb800`). Neither commit is an ancestor of `main` @ `40034b6` yet, which matches the plan's "implementation begins after that branch is merged" status; `main` and the draft plan-212 worktree are both at schema v7, so v9 after plan 208's v8 is currently free.

### What changed and what I verified

v1.6 is a reconciliation revision: it replaces Phase 0's speculative API discovery with a merge-ancestry check and rewrites the plan's seams against the implemented plan-208 code. No policy changed. I verified each new concrete claim rather than trusting the revision note:

| v1.6 claim | Verified on `cd7ee88` |
|---|---|
| `ReviewBudgetCommandContext extends RecordCommandContext { store: ReviewBudgetStore }`; `createReviewBudgetContext(...args: Parameters<typeof createRecordContext>)`; `recordPlanReviewerStepWithSnapshot(params, pending, ctx)` (`src/commands/review-budget-context.ts`) | ✅ Exact. The paired writer allocates the snapshot UUID inside its `extraOps` callback via `createReviewBudgetId()`, so §6.1's relocation of that allocation into `applyPlanReviewBudget` describes the real seam. `matchingSnapshot` (the same-step-tuple lookup §6.1 tells the implementer to reuse) already exists in `apply.ts`. |
| `applyPlanReviewBudget(input: ApplyPlanReviewBudgetInput)` short-circuits on `input.config.mode === "off"` before reading the baseline, derives thresholds from current `input.config`, and passes `B: baseline.b` to `deriveBudget` | ✅ Exact. These are precisely the three current-config-only behaviors §6.1 replaces with baseline-pinned `mode`/`configSnapshot`/folded `B`. |
| `finalizeAndWritePreparedStep(prepared, ctx, { mode, extraOps })` returns `{ finalized, recorded, stepLine, dbResult }`; in `paired-all-new` mode a `created:false` with no step line throws `RECORD_PAIR_CORRUPT` *before* the omitted-iteration retry; the generic `human-step` mirror lives only in `recordStepInternal`'s `extraOps` | ✅ Exact (`run-v1.handler.ts:2373–2534`, `2645–2675`). `AtomicAppendIfAllNewResult` carries `duplicates: Array<{ index; line }>`, so the seam can identify which op collided as §5.2 requires. The plan's three-way split (extra-key-only → `coupled-key-exists`; step exists → current duplicate behavior; pure step-key omitted-iteration collision → bounded N+1 retry) is the correct decomposition of that branch. |
| Migration v8 = `review_budget_baselines` / `review_budget_snapshots`; baseline row has no `mode`; snapshot `derived_json` is nullable | ✅ Exact. `ensurePlanReviewBaseline` strips `mode` out of `configSnapshot`, which is why the additive `mode` column and `decodeBudgetBaselinePayload` default are needed. `ALTER TABLE … ADD COLUMN … NOT NULL DEFAULT 'advisory' CHECK (…)` is valid SQLite for v9. |
| `ENFORCED_REVIEW_BUDGET_WARNING`; `ReviewBudgetState.enforcement_implemented: false` (literal); `buildReviewBudgetState` receives `mode` from callers that pass `config.reviewBudget.mode`; `template.handler.ts` guards the reviewer render on `config.reviewBudget.mode !== "off"`; reserved-enforcement copy in `src/config.ts:168` and `5x.default.toml:77` | ✅ Exact. §6.1/§6.3 name every one of these surfaces. |
| `src/records/index-rebuild.ts` loads only `steps.jsonl` and `void`s its optional `recordStore` | ✅ Exact (`:309`, `:484`). `reindexReviewBudget(recordStore, db, runId?)` already exists as the pattern for `reindexReviewGovernance`; `5x records index` is a real command. |
| `StepRecordPayload` has no role field; `hasPriorPlanReviewerStep` classifies by `phase === "plan" && step_name.startsWith("reviewer:")` | ✅ Exact. This corrects Addendum 5's `role=reviewer` shorthand into the actual classifier the plan reuses. |
| `getFileDiffSummary(..., maxLines = 200)`; `RecordStore.getLine` / `atomicAppendIfAllNew`; `PromptStore.answerPrompt(id, answer: string, answeredBy)`; no existing `review` command; `src/index.ts` withholds `createReviewBudgetIndex` while exporting `reindexReviewBudget` | ✅ Exact. |
| Cited line ranges (`protocol.ts:21–107`, `protocol-emit.handler.ts:87–267`, `protocol.handler.ts:500–657`, `protocol.ts:102–201`, `invoke.handler.ts:730–888`, `template-vars.ts:329–425`, `reviewer-plan.md:19–126`, `reviewer-plan-continued.md:13–56`, `prompt.handler.ts:225–257`, `record-store.ts:18–41`, `run-v1.handler.ts:2649–2674`) | ✅ Each range starts and ends on the construct named. |
| Surface Snapshot "Production files: 47" | ✅ Matches my count of non-test, non-doc rows in Files Touched. |

I also re-checked the areas most likely to regress after a wholesale rename (`forecastId` → `snapshotId`): every occurrence in live spec text now reads `snapshotId`, and the only `forecastId` mentions left are in the historical Revision History and Addenda, where they are correct.

**Delivery-budget assessment (first active review for this run):**
- **Ledger hygiene:** stable IDs `W1`–`W7`, `W9`, `W10`; W8 intentionally vacant; every `Addresses` entry cites a finding that is closed (P0.1–P0.4, P1.1–P1.10, P2.1–P2.6). No negative architecture rows and no `DCn` claims, so there are no credit assessments to emit. The captured baseline is `B0 = 47`, equal to the ledger sum.
- **Independent estimate `I` = 50** (medium confidence). Per row: W1 5, W2 8, W3 5, W4 5, W5 8, W7 3, W9 5, W10 3 — all consistent with the author. W6 I score at 8, not 5 (see P2.8). Direction against `B0 = 47`: aligned, well inside the 25 % / 2-point disagreement threshold; this is a scoring-honesty note, not a dispute.

### Remaining concerns

Both are polish; neither changes a contract, a route, or a phase gate.

**P2.7 (new) — `enforcement_implemented` value for advisory-pinned runs is unstated.** §6.1 says to change the field "from literal `false` to `true` for active enforced governance" and Files Touched/Tests reference `run-state-review-budget*.test.ts`, but the text does not say what an advisory-pinned active run reports. Two implementers could reasonably emit `true` ("enforcement is implemented in this CLI") or `false` ("this run does not enforce"), and their tests would disagree. Fix (one line in §6.1): type the field `boolean` and define it as `pinnedMode === "enforced"` — `false` for advisory-pinned, `uninitialized`, and `v1_compat` states, which preserves the existing literal-`false` contract for every pre-slice state. *(auto_fix; effort 0; architecture 0; high confidence.)*

**P2.8 (new) — W6 is under-scored after reconciliation.** v1.6 made W6 concrete, and its rationale now lists: extending `ApplyPlanReviewBudgetInput` and reworking mode/threshold/`B` sourcing in `applyPlanReviewBudget`; adding `mode` to three types, the codec default, `ensurePlanReviewBaseline`, and the v9 index round-trip; relocating snapshot UUID allocation; extending `FindingDelta`, `PendingBudgetSnapshot`, `BudgetSnapshotPayload`, `ReviewBudgetSnapshotRecord`, codecs, facade, and index projection with six field groups; the `applyPlanReviewGovernance` composition and post-record gate projection; one shared writer for both protocol and invoke; the §6.3 context projection for reviewer *and* author renders plus the `template.handler` guard change; removing the warning and updating run-state, `config.ts`, and the default TOML; and regression coverage across eight named test files. That is more surface than W3 or W4 (both 5) and comparable to W2/W5 (8). "Effort remains 5 because these are additive fields and one composition path" argues zero architecture delta, which I agree with, but not effort. Rescore W6 to 8 with a one-line rationale; the ledger becomes 50, still under the standard ceiling. My `I` already reflects this, so the item carries no effort delta of its own. *(auto_fix; effort 0; architecture 0; medium confidence.)*

### Updated readiness

- **Plan-review governance plan completion:** ✅ Complete. v1.6 replaces every speculative plan-208 reference with verified concrete seams and introduces no policy regression; all findings from the initial review and Addenda 1–5 remain closed.

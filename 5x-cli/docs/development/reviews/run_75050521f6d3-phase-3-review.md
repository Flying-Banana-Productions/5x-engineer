...
## Addendum (2026-09-22) — Phase 3 review findings addressed

**Reviewed:** `7c394a3f2afb330b413f51e19b7089307a2a5ef4` (on top of `03354d2`, the commit that recorded the original review above)

**Local verification:** `bun test test/unit/review-governance test/integration/plan-diff.test.ts test/integration/commands/protocol-validate.test.ts test/unit/commands/protocol-emit.test.ts test/integration/commands/review-budget.test.ts` — 79 pass, 0 fail (up from 73). `bunx tsc --noEmit` — clean. `bunx biome check src test` — clean. I re-ran the original P1.1 repro directly against `buildPlanReviewDiffContext` with `workdir` set to `5x-cli/` (a subdirectory of the git top-level) over the real `2b11d9a..ea40ac0` range: it now returns 2 hunks / 6143 bytes of patch (previously 0/0), and the rendered retrieval command (`git diff 2b11d9a..ea40ac0 -- ':(top)5x-cli/docs/...'`) runs correctly whether executed from the repo root or from `5x-cli/`.

### What's addressed (✅)

- **P1.1 — Plan diff empty below git top-level**: **Addressed.** `planPathspecs` now returns `root` alongside `displayPath`/`pathspecs`, and `buildPlanReviewDiffContext` runs `--name-status`, `planPatch`, and the `rev-list` calls against `root` instead of `workdir`. The rendered retrieval command uses `:(top)<path>` pathspec magic so it works regardless of the caller's cwd. A new integration test (`"builds a top-level-relative plan diff from a subdirectory workdir"`) creates a nested `packages/app` workdir and asserts the patch and rendered command are correct. Verified independently against this repository's actual layout — fixed.
- **P2.1 — Diff-context build failures swallowed**: **Addressed.** `PlanDiffFailure` (a `Pick<PlanDiffError, "code"|"message">`) is now threaded through: `protocol.handler.ts` captures the thrown error into `diffContextFailure` (normalizing non-`PlanDiffError` throws to `PLAN_DIFF_GIT_ERROR`) and passes it into `validateClosureReview`; `closure.ts`'s `validateIntroducedByFields` includes the code and message in the `PLAN_DIFF_CONTEXT_MISSING` diagnostic when a failure is present. `template-vars.ts` now renders `formatPlanReviewDiffFailure(...)`, a dedicated prompt section with the commit range, error code, and message, instead of returning `diffAppend: null`. Both paths are covered by new unit tests (`closure.test.ts`, `plan-diff.test.ts`).
- **P2.2 — Mode not pinned**: **Addressed as an explicit, documented deferral** (this is the correct resolution, not a workaround). The plan checklist item 3.3 now reads "load the currently available active mode... **Phase boundary:** Phase 3 reads mode from active config because the plan-208 baseline payload does not carry it; Phase 6.1 adds and consumes the durable baseline-pinned mode before enforced governance is complete." A matching code comment was added at the `budgetContext.config.reviewBudget.mode` read site in `protocol.handler.ts`. This converts the original overclaim into an accurate, traceable statement of current scope; the underlying behavior (reading live config) is unchanged, but that's appropriately scoped to Phase 6.1 rather than Phase 3.
- **P2.3 — Straddling hunks not listed as omitted**: **Addressed.** `formatPlanReviewDiffContext`'s omitted-header logic now computes each hunk's end line (`start + hunk.text.split("\n").length`) and includes the header when `start >= maxLines || end > maxLines`, using a `searchFrom` cursor to avoid `indexOf` re-matching an earlier occurrence of an identical header. New unit test `"lists a hunk whose body straddles the truncation boundary"` confirms a hunk starting before the cutoff and ending after it is now listed.
- **P2.4 — Unbounded equivalent-commit scan**: **Addressed.** The builder now finds `lastPlanCommit` via a single `git rev-list -1 <range> -- <pathspecs>` call, diffs only that one commit against `patch`, and if equivalent, slices `revisions` from that commit forward (covering later review-artifact-only commits) rather than diffing every commit in the range. When no commit in the range touches the plan, all revisions are treated as equivalent (correct, since the plan didn't change). New integration test `"limits equivalent endpoints to the last plan-touching commit and later artifacts"` exercises a range with two separate plan revisions and confirms only commits from the *last* plan revision onward are treated as equivalent — the earlier plan revision and its artifact commit are correctly excluded.
- **P2.5 — Closure string fields not type-checked**: **Addressed.** `assertReviewerVerdict` in `protocol.ts` now loops over `["failure", "lowestCostCorrection", "lateDiscoveryEvidence", "priorDecisionId", "newEvidence"]` and fails with a field-specific message if the value is present but not a string. New unit test `"rejects non-string closure evidence fields"` in `protocol-emit.test.ts` parametrizes over all five fields and confirms each is rejected with `INVALID_STRUCTURED_OUTPUT` and a message containing the field name.

### Remaining concerns

None. All six items from the original review (one P1, five P2) have direct, verified fixes with accompanying tests. No new issues were introduced by this revision — the diff is additive/corrective and stays within the same modules touched in the original commit. `equivalentPlanCommits` semantics changed subtly (now derived from the last plan-touching commit rather than an exhaustive per-commit scan); I checked this against the plan's requirement ("Accept an end commit only when its plan-only patch is byte-equivalent to the rendered context, allowing a later review-document-only commit") and the new logic satisfies it correctly while being bounded.

### Updated readiness

- **Phase 3 completion:** ✅ — All identified issues are resolved with passing tests, clean `tsc`, and clean `biome`. Plan checklist items in section 3.1–3.3 are accurate (no overclaiming).
- **Ready for next phase:** ✅ — Phase 4 (deterministic enforced routing) can proceed without qualification.

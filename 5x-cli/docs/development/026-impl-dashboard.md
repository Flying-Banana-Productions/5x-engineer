...
Desired behavior: `5x dashboard` starts a token-protected Bun HTTP/WebSocket server (default `127.0.0.1:55555`) that renders a dense, real-time mission-control UI backed by the v1 schema (`plans`, `runs`, `steps`) and `.5x/logs/` NDJSON files. It supports multi-plan drill-down, run/step timeline streaming, live log tailing, quality gate result summaries, cost/token analytics, and historical comparison — all without schema changes.
...
**Dashboard remains a standalone process, not embedded in run commands.** Decouples monitoring from execution. Allows observation before, during, and after runs. Requires control-plane root resolution to find the correct `.5x/` directory from any checkout context. Trade-off: extra process management, but avoids coupling the dashboard lifecycle to any single run.
...
**Control-plane root resolution required.** The dashboard must resolve the canonical `.5x/` location via the same `resolveControlPlaneRoot()` path used by other commands (`src/commands/control-plane.ts`), handling managed/isolated/none modes and linked worktrees. Trade-off: imports control-plane resolution logic, but ensures correct DB discovery from any checkout directory.
...
- [ ] Implement `src/commands/dashboard.handler.ts` with control-plane root resolution via `resolveControlPlaneRoot()`, DB path discovery, and delegation to `startDashboardServer()`.
...
- [ ] Open dashboard DB via `openDbReadOnly(controlPlaneRoot)` in `src/dashboard/server.ts`. Verify connection health on startup with a probe query.
- [ ] Generate per-session token (`crypto.randomBytes(32).toString("hex")`), persist to `.5x/dashboard-token.<port>.json` (mode `0600`) with `{ pid, port, token, startedAt }`. Clean up token file on shutdown.
...
- [ ] Handle fresh-project bootstrap: if `.5x/` or DB is absent, server still starts, serves app shell, returns empty snapshot payload with `status: "no_project"`.
...
## Phase 2: Data Snapshot + WS Protocol

**Completion gate:** initial `snapshot` payload renders complete plan/run/step state; incremental polling emits scoped WS updates every 2s; protocol is versioned and typed.

- [ ] Implement dashboard data module in new `src/dashboard/data.ts` with queries against the v1 schema (`plans`, `runs`, `steps` tables).
- [ ] Implement `buildSnapshot()`: list all plans, list runs per plan (with computed summaries via `computeRunSummary()`), fetch recent steps for active runs. Return typed `DashboardSnapshot`.
...
- [ ] Implement `getRunDetail(runId)`: full step history with phase grouping derived from `step_name`/`phase` columns, quality gate results (`step_name = 'quality:check'`), and cost/token aggregates.
- [ ] Define WS protocol envelope (`v: 1`) and typed message unions in new `src/dashboard/ws-protocol.ts`.
- [ ] Implement polling loop (2s interval) in `src/dashboard/poller.ts` — polls `steps`, diff against last snapshot, emit scoped WS messages to connected clients.
...
**Completion gate:** active log files stream to subscribed WS clients in near-real-time; missed `fs.watch` events recovered by periodic rescan; backfill endpoint serves historical lines.
...
- [ ] Watch `.5x/logs/<run-id>/` directories for `agent-*.ndjson` files. Discover new run log dirs from `steps` rows with non-null `log_path`.
...
- [ ] Add HTTP endpoint `GET /api/logs/:runId?file=agent-NNN.ndjson&offset=0&limit=500` for backfill/history, supporting pagination via byte offset. Enforce max 10,000 lines per response.
...
- [ ] `data.test.ts` — snapshot composition from seeded DB, `pollSteps` cursor correctness, `getRunDetail` phase grouping, empty-DB handling.
- [ ] `ws-protocol.test.ts` — message serialization/deserialization, unknown message type handling, version mismatch rejection.
...
- [ ] `poller.test.ts` — step diff computation, run status inference from terminal steps, incremental update message construction.
...
- [ ] `dashboard.test.ts` — fresh project: missing `.5x/` returns app shell with `no_project` snapshot.
...
| `src/commands/dashboard.handler.ts` | New handler: control-plane resolution, server startup, signal-owned shutdown. |
| `src/dashboard/server.ts` | New Bun HTTP/WS server lifecycle, auth enforcement, token file management. |
| `src/dashboard/routes.ts` | New static asset serving (embedded + dev filesystem fallback), API routes. |
| `src/dashboard/data.ts` | New read-only queries against `plans`/`runs`/`steps`, snapshot builder, step poller. |
| `src/dashboard/ws-protocol.ts` | New protocol message types and validation. |
| `src/dashboard/poller.ts` | New polling loop: step cursor, diff, WS broadcast. |
...
| `test/unit/dashboard/*.test.ts` | New unit tests for data, ws-protocol, log-watcher, poller. |
| `test/integration/commands/dashboard.test.ts` | New integration tests for CLI startup, auth, shutdown. |

## Tests

| Type | Location | Validates |
|------|----------|-----------|
| Unit | `test/unit/dashboard/data.test.ts` | Snapshot composition, cursor polling, phase grouping, empty DB. |
| Unit | `test/unit/dashboard/ws-protocol.test.ts` | Message schema, version handling, unknown type rejection. |
| Unit | `test/unit/dashboard/log-watcher.test.ts` | Offset reads, rescan recovery, partial-line buffering, subscriber cleanup. |
| Unit | `test/unit/dashboard/poller.test.ts` | Step diffing, run status inference, incremental message construction. |
| Integration | `test/integration/commands/dashboard.test.ts` | CLI startup/shutdown, auth enforcement, token file lifecycle, fresh-project bootstrap. |
...
- **Multi-repo aggregation.** One dashboard process monitors one project's `.5x/` directory.

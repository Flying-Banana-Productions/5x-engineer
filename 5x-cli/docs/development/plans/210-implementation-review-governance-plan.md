...
| Rebuild | Budget projections | Migration v9 and `review-governance/sqlite-index.ts` extend rebuilding to decisions/gates; migration v10 (`src/db/schema.ts:589–596`) persists plan-review closure context in the budget index. `records/index-rebuild.ts` already dispatches budget/decision projection. Do not reimplement it as steps-only. |
...
**Approval must survive a separate execution run, and binding is not optional for a budgeted admission.** At the first implementation-phase author admission, if the plan has a `## Delivery Budget` and resolved config mode is not `off`, require a binding even when this execution run has no baseline. Auto-resolve the unique approved source run for the canonical plan path through RecordStore/progress resolution; zero or multiple approved candidates return `IMPLEMENTATION_APPROVAL_REQUIRED`. Explicit `review implementation bind --source-run` resolves selection without relaxing approval validation. Same-run and separate-run sources use the identical validator. Plans without a budget or first admissions configured `off` retain v1 behavior; an existing binding always retains its inherited pinned mode despite later config edits. Carry the source baseline identity, pinned mode/thresholds, approved ledger, effective decisions and evidence hashes into a self-contained immutable record. This is inherited state, not baseline capture or a rescore. Refuse an unapproved/drifted plan. Approval after a plan final-correction pass must include the recorded final author commit and the source's completed plan-review lifecycle, not merely its last `final_corrections` route.

**Do not loosen plan 209's hunk validator to implement code review.** Add a sibling code-diff context. Capture the pre-delegation HEAD durably at the phase's first implementation author admission and retain it across retries; that is the initial review base, including every commit in a multi-commit author session. Before reviewer delegation, record exact full SHAs, per-file complete hunks, the canonical patch hash and excluded workflow artifacts. The reviewed end is the recorded author commit, not HEAD after a reviewer writes documentation. Continued start is the previous review context's reviewed end, not its review-artifact commit. For legacy history without a captured base, use the parent of the earliest same-phase `git:commit` record only when unambiguous and ancestry-verifiable; otherwise fail explicitly. Exclude only configured run-record paths, the exact plan, and registered review artifacts; documentation/config/tests elsewhere remain implementation changes. A hunk citation includes its `diff --git` file header and complete `@@` hunk to disambiguate identical hunks in different files. Preserve source whitespace; do not reuse the plan validator's trailing-whitespace stripping for whitespace-sensitive code.
...
**Text authorization has explicit spans and durable lineage.** `planImpact` is an object `{ kind, locations: [{ heading, staleText }] }`, not a string enum. For `text_only`, locations must be nonempty and every literal `staleText` must occur exactly once under its uniquely identified heading at the approved text anchor. Initially that anchor is the approved plan commit; after a successful guarded amendment it is the last CLI-verified amendment commit in that binding's authorized lineage. Missing/ambiguous matches route to a human, not a best-effort replacement. Only those byte spans and checkbox toggles may differ, with the table and structural exclusions still protected. Successful verification appends before/after commit/blob hashes, guard identity and parent-lineage reference; it does not replace the approved binding or rescore scope. All later drift checks replay this chain from approved bytes and compare against the resulting authorized bytes, ignoring checkbox state. This permits a legitimate text-only fix to reach `phase:complete` without re-binding; edits outside that lineage still require a human amendment workflow.
...
**A verified correction may carry assessments forward, not invent them.** Target-phase code changes ordinarily invalidate earlier claim assessments. The sole no-review exception is the originating observation's assessments carried to its correction commit through a CLI-recorded passing, still-eligible correction attempt. Its proof must include zero architecture delta, empty boundary changes, a clean changed-path inventory confined to the eligible fix, and fresh full quality success. Carry only the existing claim IDs/deltas/evidence, with source observation and destination commit references; missing due assessments or existing shortfall gates remain unresolved. Failed/invalidated attempts, extra code changes, dirty inventories or uncertain boundaries cannot carry assessments and require reviewer re-entry.
...
|---|---|---:|---:|---|---|---|
| W1 | Bind execution to approved plan provenance and inherited ledger | 5 | 2 | - | P1.1, P1.3 | Adds a versioned durable approval reference and cross-run read boundary; first-admission triggering and authorized-text lineage extend its existing validation/tests without a new subsystem. |
| W2 | Add phase-selected implementation protocol and pure classification rules | 3 | 1 | - | P1.5 | Threads a distinct public item/realization contract through schema, emitter and contextual validation, including concrete planImpact locations. |
| W3 | Capture exact implementation review ranges and validate code hunks | 5 | 1 | - | P1.2 | Adds persisted review context plus git evidence boundary; captures pre-author HEAD using the existing admission path and tests multi-commit sessions/legacy fallback. |
...
| W5 | Validate plan impacts and protect text-only amendment bytes | 5 | 1 | - | P1.3, P1.5 | Adds durable amendment guards, exact authorized spans and verification lineage within the existing committed-byte/structure comparison. |
| W6 | Complete eligible corrections through fresh full quality evidence | 5 | 1 | - | P1.4 | Adds a commit/config-bound correction attempt and quality contract; existing proof now explicitly governs claim carry-forward as well as failure/retry routing. |
| W7 | Reconcile approved debt claims and derive realized budget state | 5 | 1 | - | P1.4 | Extends inherited budget derivation and durable observations; restricts carry-forward to verified correction proof without a new credit authority. |
| W8 | Extend typed gates, decision folds and rebuild for implementation | 5 | 1 | - | P2.1 | Extends shipped CAS and domain/phase projections with tests; corrects the additive migration to v11 after shipped v10, not another migration. |
| W9 | Enforce phase admission/completion and final-run guards | 3 | 0 | - | P1.1, P1.2, P1.3, P1.4 | Threads one predicate through existing paths, including first binding/base capture, authorized text lineage and verified credit carry-forward bypass regressions. |
| W10 | Update implementation prompts and all delegation-mode skills | 3 | 0 | - | P1.1, P1.2, P1.5 | Existing workflow/assets include mandatory binding, pre-author capture and the concrete planImpact object; rendered tests remain in this item. |
| W11 | Publish run-state telemetry, integrated acceptance and canonical docs | 3 | 0 | - | P1.1, P1.2, P1.3, P1.4, P1.5, P2.1 | Existing acceptance lifecycles cover the corrected admissions, text-lineage and realized-claim shortcut; docs name the corrected contracts/migration. |
...
Review v1.1 retains each item's effort and architecture score: these corrections specify missing conditions/data on the already-scored binding, context, guard, proof and migration seams. No new production file, subsystem or independent authority is added; the incorporated review IDs above identify the associated regression work.
...
approved plan review (209): baseline + snapshot + final author/decision history
                 |
       mandatory first-admission binding (unique source or explicit selection)
                 |
...
SQLite v11 projections = rebuildable view; no second budget or decision authority
```
...
interface PlanImpact {
  kind: "text_only" | "design" | "budget";
  locations: Array<{ heading: string; staleText: string }>;
  // text_only: nonempty, unique nonoverlapping literal spans at approved text anchor.
  // design/budget: locations may be empty; neither qualifies for the exemption.
}

interface PhaseAuthorAdmission {
  bindingId: string;
  phase: string;
  preAuthorCommit: string; // full HEAD SHA captured before first author delegation
  // Durable first-admission step payload; retries must not reset the phase base.
}
...
**Files:** new `src/review-governance/implementation-state.ts`; extend `review-budget/record-lines.ts`, `control-plane/review-budget-store.ts`, `commands/{review,review-decision.handler,template.handler,invoke.handler,run-v1.handler}.ts` and `records/index-rebuild.ts`. Phase 1 defines the shared first-admission binding check; Phase 9 completes its boundary/bypass wiring.
...
- [ ] Add `5x review implementation bind --run <execution> --source-run <plan-review>` using shared command context. At first implementation-phase author admission with no existing binding, inspect the plan and resolved config: `## Delivery Budget` plus mode other than `off` requires binding before work is delegated/admitted. Resolve the unique approved source run for the canonical plan path through RecordStore/progress resolution; zero or multiple candidates return `IMPLEMENTATION_APPROVAL_REQUIRED` with explicit-selection remediation. Validate canonical plan identity, terminal approval/final-correction evidence, no open gate, and the approved commit for both automatic and explicit selection. Repeated identical binding is a no-op; conflicting binding needs explicit human-approved amendment provenance.
- [ ] Detect plan drift against the approved bytes plus the ordered chain of Phase 5 guard-verified text amendments; ignore checkbox-state changes. Initially the chain is empty. Each successful verification extends the authorized text anchor without altering approved ledger/design authority or requiring re-binding; an unverified/malformed chain never authorizes drift. New work items/claims, changed scores, acceptance/design intent or phase structure still require amendment. Map `phase-N` debt targets to numeric parsed IDs; accept exact numeric targets; reject unmatched/ambiguous arbitrary labels before activation instead of assuming numeric sort identifies the dependency.
- [ ] Plans without a budget or first admissions with resolved mode `off` remain v1; persist that first-admission compatibility disposition rather than infer v1 from absence of an execution-run baseline. Existing bindings keep source-pinned mode/thresholds across config edits. Historical unbudgeted compatibility runs need explicit approved opt-in, not baseline inference. No new global configuration switch.
- [ ] Unit tests cover the first-admission budget/config matrix, zero/one/multiple approved candidates, immutable lineage, final-correction approval, imported decisions, duplicate/conflicting bind, unknown phases and kind filtering. Integration tests cover unbound separate execution, source worktree removal/rebuild, changed-plan refusal, and (with Phase 5) successful text-lineage drift validation without re-binding.
...
- [ ] Add the implementation enum and required priority/deltas; conditional nonempty unique `planWorkItemIds` and required `PlanImpact` object for `plan_defect`. Its exact shape is `{ kind: "text_only" | "design" | "budget", locations: [{ heading, staleText }] }`; reject string-valued impacts, unknown fields/kinds, invalid location shapes and blank heading/staleText. `locations` must be nonempty for text_only; design/budget may use an empty array and always route human. Validate W IDs against the bound approved ledger; prohibit `planImpact` on other classes and prohibit implementation `creditClaim`, `creditAssessments` and `baselineAssessment`. Extend `--item` parsing and repeated `--credit-realization '<json>'` with nonempty evidence and strict shapes; keep aggregate-key rejection recursive.
- [ ] For each text-only location, resolve the exact heading uniquely and require literal staleText to occur exactly once within that heading's section at the approved text anchor (approved commit, extended only by verified Phase 5 lineage). Resolve byte offsets CLI-side; reject overlapping/duplicate locations and spans intersecting protected table/structure. Missing or ambiguous matches produce a human-route diagnostic, never an automatic replacement. Structural shape errors reject input; contextual ambiguity disables the exemption. Standalone validation cannot assert span authorization.
...
- [ ] Test each field's missing/invalid/duplicate case, concrete PlanImpact object round-trip, empty text locations, missing/duplicate headings or staleText, overlapping/protected spans, arbitrary signed integer telemetry versus plan allowed magnitudes, cross-domain fields, unknown W IDs, plan-defect priority over bug symptom, critical safety, and legacy off/advisory/enforced compatibility.

## Phase 3: Exact code-review context and convergence evidence (W3)

**Completion gate:** The initial range starts at the durably captured pre-author HEAD and includes every commit in a multi-commit author session. Prompt and recorded verdict reference one exact fix range; wrong-range, wrong-file or assembled hunk evidence cannot create an ordinary enforced blocker.
...
- [ ] At the phase's first implementation author admission, template render or invoke records `PhaseAuthorAdmission.preAuthorCommit` from pre-delegation HEAD through the existing durable step path, after binding and before author work. Capture once per binding/phase; retries, multiple renders, new sessions and quality retries reuse it rather than advancing it. Use this as the initial review base and verify it is an ancestor of the reviewed author commit, including all intervening same-session commits even though AuthorStatus returns only its final commit.
- [ ] For legacy history without a captured pre-author base, use the parent of the earliest same-phase `git:commit` record in authoritative step order, with commit/phase identity and ancestry verified. Never use the parent of the author-result commit as a substitute. Missing/unresolvable/ambiguous merge or root-parent history returns an explicit context error; a post-work direct author record must not stamp current HEAD as a fictional pre-author base. Continued base remains the previous observation's reviewedCommit; end is the latest admitted correction commit.
...
- [ ] Unit tests use injected git adapters/pure patches. Integration tests include two or more `git:commit` steps in one author session with only the last commit in AuthorStatus, idempotent pre-author capture across renders/retries, legacy earliest-git-commit fallback and missing fallback failure. Also cover review-only commits, renamed/deleted files, identical hunks in two paths, CRLF/whitespace-sensitive code, truncation, empty ranges, bad refs, dirty worktrees, fresh sessions and record/rebase drift.
...
**Completion gate:** Only exact authorized text spans can change without a plan decision; successful guard verification extends durable authorized drift lineage so completion does not require re-binding. Every budget-table byte change, out-of-span edit or structural/ambiguous amendment invalidates the exemption.
...
- [ ] Route on `planImpact.kind`, not a string-valued planImpact. Derive text_only as ordinary author correction with a durable guard attached to the originating observation. Resolve each `{ heading, staleText }` against the approved text anchor to unique nonoverlapping byte spans and save anchor commit/blob hash, parent-lineage identity, exact table bytes, allowed spans and structural signature **before** delegation. Missing/ambiguous text or heading routes human; reject duplicate/missing tables and ambiguous section bounds. Do not use `originalSection` or parsed ledger equality as a byte-preservation proxy.
- [ ] On the author's recorded commit verify the guard against the committed plan blob and current mapped worktree; no uncommitted plan changes may serve as evidence. Compare Buffer bytes including CRLF/trailing spaces. Permit replacements only within the saved spans, plus checkbox toggles; unchanged surrounding byte segments must match in order even when replacements change length. Table/phase/debt/design/acceptance protections take precedence over location authorization. Out-of-span edits, missing guard or mismatch emit typed amendment failure and block exemption consumption.
- [ ] Append successful verification to the existing implementation-state record facade, linking binding/guard/source observation, parent lineage, before/after commits and blob hashes. Rebuild the authorized plan bytes by verifying the chain from the original approval; Phase 1 drift checks and Phase 9 boundaries use this effective text anchor with checkbox-only differences ignored. No new plan approval/baseline or re-binding is required. Unverified commits, broken chain links, stale guards and unrelated edits still require amendment. Do not mutate the original approvedPlanHash.
- [ ] Derive design/budget kinds (and uncertain text-only classification) as human gates with `nextAction: plan_amendment`. A changed table always requires budget impact review, even if parsed values are equal. Never automatically edit the ledger, add Addresses links for implementation telemetry, or silently downgrade a design defect to implementation work.
...
- [ ] Test same-code-plus-text pass through normal review closure to `phase:complete` without re-binding, chained verified edits/restart/rebuild, missing or ambiguous staleText, duplicate headings, length-changing replacements and edits just outside permitted spans. Also cover one whitespace/table/Addresses/debt-cell change, removal/duplicate tables, changed phase heading/checklist identity, design edits outside table, invalid bytes, dirty plan, broken lineage, amendment restart and attempted post-approval claim creation. Final correction shortcut remains ineligible for every plan defect.

## Phase 6: Quality-backed final implementation corrections (W6)

**Completion gate:** Only an eligible, unchanged-boundary correction with a fresh passing full suite reaches phase gate without reviewer re-entry. Its CLI proof may carry the originating observation's claim assessments to that correction commit; a failed attempt cannot regain the shortcut or carry-forward by retrying.
...
- [ ] Persist failed as well as passed attempts and quality rerun telemetry. Any gate failure/timeout or skipped gate durably invalidates the shortcut for that observation; normal configured quality retries may continue but a later pass still routes to reviewer re-entry. Missing results or crash during execution reruns the full suite; it never guesses success. Any further code/config change beyond the attempt's bound correction commit creates new evidence and requires re-entry.
...
- [ ] In a passing attempt persist explicit zero architecture delta, empty boundary changes and clean changed-path inventory confined to the eligible correction, together with source observation/assessed commit and destination correction commit. Only this still-eligible CLI proof carries the originating per-claim assessments forward unchanged. Missing due assessments remain missing; partial/not-realized assessments and their gates remain unchanged. Any extra code change, dirty/unknown inventory, boundary uncertainty or previously invalidated attempt requires reviewer re-entry and cannot carry assessments.
...
**Completion gate:** Every due approved claim has a bounded observation valid at the reviewed commit (or carried only through Phase 6's exact eligible proof), or an explicit governing waiver. Derived credit/alerts reflect realizations while implementation variance remains telemetry only.
...
- [ ] Validate per-claim realization and concrete evidence referencing the implemented post-state/commit. Retain full claimed negative delta for realized; strict smaller negative magnitude for partial; zero for not realized. Never allow an implementation finding's negative architecture delta or an unknown claim to affect credit. Changed target-phase code after assessment invalidates its completion authority until reviewed again **except** for the originating observation's assessments carried to the exact correction commit by Phase 6's CLI-recorded passing, still-eligible attempt (zero architecture delta, empty boundary changes and clean changed-path inventory). Validate that proof and its source/destination identities in reconciliation; author claims or generic quality passes cannot substitute. All other code changes invalidate prior assessments.
...
- [ ] Test no claims, future-only claims, all realized, partial/not-realized, multiple contributions and caps/rounding, no credit creation, ordinary variance invariance, restored claims, accepted waiver, large shortfall within budget and small shortfall above E. Verify realized-claim carry-forward only with the exact eligible CLI passing proof; reject wrong source/commit, generic quality success, invalidated attempt, extra code/architecture/boundary changes and dirty inventory. Missing claims and shortfall gates cannot be repaired by carry-forward.
...
- [ ] Add additive migration v11 (or next available after integration), following dependency v10's plan-review closure-context migration, for implementation binding/observation projections; extend existing gate/decision projection payloads. UUID/text identities, per-stream order, and standard redacted origins only. Existing PromptStore typed notification protection is reused; no new authoritative gate table. Extend `records index` dispatch for implementation kinds and preserve unknown-version diagnostics/fail-closed enforcement.
...
**Completion gate:** An unbound budgeted execution cannot evade first-admission binding. Enforced execution cannot bypass reconciliation/gates/quality through manual checklists, generic record calls, alternate templates, composites or sealing; verified text lineage and eligible claim carry-forward still permit legitimate completion.
...
- [ ] Before first implementation author delegation/admission, run Phase 1's shared binding check in both template and invoke, then Phase 3's durable pre-author HEAD capture. Do not equate an absent execution-run baseline with v1: a budgeted plan plus non-off config must auto-bind a unique approved source or fail `IMPLEMENTATION_APPROVAL_REQUIRED`. Generic direct author record admission runs the same binding check, and if pre-author capture is absent it may use only the verified legacy git-commit fallback, never post-work HEAD. Persisted first-admission compatibility/binding state prevents later accidental mode changes.
- [ ] Call the prior-phase boundary form from both template and invoke for next-phase author work and from record admission for manually recorded author steps. Raw `phase:complete` or attempted implementation-run sealing with implementation history cannot bypass the same binding requirement by omitting author render. Existing parser has no dependency DAG; use ordered phases conservatively: any unreconciled claim due in an earlier phase blocks advancement. Do not invent a generic phase dependency framework. Same-phase repairs remain allowed.
...
- [ ] Integration tests exercise an unbound separate execution run under non-off config (unique/zero/multiple approved sources), raw `run record phase:complete`, manual checked boxes, first/next-phase native/invoke rendering, direct author recording without pre-author capture, skipped checklist validation, run completion before/after realization, zero-claim runs, last-phase shortfall and abort/reopen. Include completion after guard-verified text lineage and after eligible realized-claim carry-forward; assert all other drift blocks and advisory/v1 routing remains documented.
...
- [ ] Rewrite initial review as the exhaustive material pass and subsequent reviews as closure. Document four classes, source-of-correction precedence, exact new-blocker evidence, safety exception, work-item linkage, no new credit and nonblocking Markdown follow-ups. Providers and protocol emit examples use the concrete `planImpact: { kind, locations: [{ heading, staleText }] }` object, not a string; show nonempty text-only locations anchored to supplied approved text and human routing for missing/ambiguous matches.
...
- [ ] Update execution skill to consume mandatory first-admission binding/unique-source resolution and captured pre-author HEAD from template/invoke; zero/multiple approved sources require explicit selection, never v1 fallback. Retain review context ID, record/consume CLI route, call `review corrections finish`, return to quality retry then reviewer on invalidation, and resolve typed review gates through `review decide`. Do not treat generic `human:gate`/approve-override as an enforced bypass. Resume from durable route, not local counters or reviewer prose. Read retry limits/config, preserve freshness warnings and delegation-mode precedence.
...
- [ ] Exercise full lifecycles: separate plan/execution runs with mandatory unbound admission; a multi-commit author session reviewed from pre-author HEAD; initial defects to diff-causal closure; guard-verified text-only pass through normal closure to `phase:complete` without re-binding versus raw-byte/out-of-span violation; a single P2 full-quality shortcut in a phase with an already realized due claim carried forward only by the eligible CLI proof; failed quality followed by successful retry **with** reviewer; partial debt shortfall and human successor decisions; last-phase sealing guard; fresh-process resume and wiped-index rebuild. Check the PlanImpact object/locations across emit/render/record and migration from dependency v10 to v11.
...
| File(s) | Change / owner |
|---|---|
| `src/review-governance/implementation-state.ts` (new) | First-admission binding/disposition, pre-author base, text-verification lineage, review observation and correction-proof record facade/codecs; W1/W3/W4/W5/W6/W7. |
| `src/review-governance/implementation.ts` (new) | Contextual validation and implementation routing; W2/W4/W5. |
| `src/review-governance/code-diff.ts` (new) | Exact patch/range/hunk evidence using captured pre-author base or verified legacy earliest-commit fallback; W3. |
| `src/review-governance/plan-amendment.ts` (new) | PlanImpact span resolution, raw table/structural guard and authorized drift-lineage verification; W5. |
| `src/review-governance/corrections.ts` (new) | Shortcut eligibility/proof, claim carry-forward evidence and invalidation; W6. |
| `src/review-governance/credit-reconciliation.ts` (new) | Approved per-claim fold with exact eligible-proof carry-forward; W7. |
| `src/review-governance/implementation-boundary.ts` (new) | Single boundary predicate honoring mandatory binding, verified text lineage and correction proofs; W9. |
...
| `src/protocol.ts`, `src/protocol-normalize.ts` | Schema/union, concrete PlanImpact object/location contract and strict per-phase validation preservation; W2. |
...
| `src/commands/{template-vars,template.handler,invoke.handler}.ts` | Mandatory first binding, pre-author HEAD capture, durable review context and admission; W1/W3/W4/W9/W10. |
...
| `src/commands/{run-v1.handler,quality-v1.handler,phase.handler}.ts` | Direct-admission binding checks, telemetry, full quality/carry-forward evidence and text-lineage-aware completion guards; W1/W4/W6/W9/W11. |
| `src/db/schema.ts`, `src/records/index-rebuild.ts` | Additive v11 migration after shipped v10 and implementation projection dispatch; W1/W8. |
...
| `test/unit/review-governance/{decisions,store-index}.test.ts`; `test/unit/records/index-rebuild.test.ts`; `test/unit/db/schema-v11.test.ts` (new, adjust number if needed) | Decision ordering, lineage/proof projection and upgrade parity from shipped v10. |
...
|---|---|---|
| Unit/contract | Binding and record facade | Budget/config trigger, unique-source selection, zero/multiple-source failure, same/separate run inheritance, immutable source, compatibility, authorized text lineage, no recapture, kind isolation. |
| Unit | Protocol + implementation policy | Four classes, enum separation, PlanImpact object/location validation, unique heading/text spans, conditional fields, W IDs, human precedence, prior closure, critical exception and re-raise evidence. |
| Unit + integration | Code diff | Pre-author HEAD across multi-commit sessions/retries, earliest-git-commit legacy fallback, exact endpoints, file-qualified hunks, whitespace, rename/deletion, truncation, artifact-only commits, rebase and stale context. |
...
| Unit + integration | Amendment guard | Raw Buffer identity, unique nonoverlapping authorized spans, out-of-span/structural edits rejected, verified lineage through phase completion without re-binding, restart/rebuild and broken-chain refusal. |
| Unit + integration | Final corrections | One P2 with realized due claim, zero architecture/empty boundaries/clean inventory, fresh suite, eligible proof carry-forward only, empty/skipped config, failure latch and mandatory re-entry. |
| Unit | Credit derivation | Per-claim bounds, future/due claims, exact correction-proof carry-forward versus other drift, caps/rounding, material shortfalls, no new credit, invariant gross effort/P. |
...
| Integration | Completion guards | Unbound budgeted execution refusal/auto-bind, raw record/complete calls, first/next-phase admission, checked-box bypass, early author completion allowed, text-lineage completion, correction-proof carry-forward, final debt block, abort and compatibility. |
...
### v1.1 (September 23, 2026) — Close admission, evidence and completion gaps

Addresses actionable findings in [the v1.0 plan review](../reviews/5x-cli-docs-development-plans-210-implementation-review-governance-plan-review.md):

- **P1.1:** Require first implementation admission to bind when the plan has a Delivery Budget and resolved mode is non-off; auto-select only a unique approved source and fail explicitly otherwise. Direct recording/completion bypass tests use the same rule; v1/off compatibility is no longer inferred from absence of a baseline.
- **P1.2:** Capture pre-author HEAD once before delegation, preserving all commits in one author session. Legacy fallback uses the earliest same-phase git:commit record's parent with verified ancestry, never the author-result commit's parent. Wire the capture through existing admission paths.
- **P1.3:** Persist successful guard verification as authorized text-amendment lineage. Drift/boundary checks replay it from approved bytes without mutating the binding, and lifecycle tests reach phase completion without re-binding.
- **P1.4:** Permit existing claim assessments to carry forward only through the originating observation's exact CLI-recorded passing, still-eligible correction proof, with zero architecture delta, empty boundary changes and clean inventory. All other code changes invalidate assessments; test the shortcut in a phase with a realized due claim.
- **P1.5:** Define the concrete PlanImpact object and heading/staleText locations, exact unique byte-span resolution and human routing for ambiguity. Restrict guards to those spans plus checkbox toggles; align protocol, prompt examples and lifecycle tests.
- **P2.1:** Verify dependency migration v10 at `src/db/schema.ts:589–596` and consistently schedule the new projection migration/test as v11 (or the next free number at integration).

Stable work-item IDs and individual scores are retained; Addresses now links each incorporated finding. Corrections refine already-scored contracts and tests without new production files or independent architecture. This is a bounded documentation revision in the ordinary advisory re-review cycle, not a final-correction shortcut; no source/config/test files or dependency worktree were changed, and no tests or quality gates were run for this revision. Plan parser validation is the only execution check.

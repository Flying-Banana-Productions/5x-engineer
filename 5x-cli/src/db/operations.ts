...
import type { VerdictBlock, StatusBlock } from "../parsers/signals.js";
...
export function getPlan(db: Database, planPath: string): PlanRow | null {
  return db.query("SELECT * FROM plans WHERE plan_path = ?1").get(planPath) as PlanRow | null;
}
...
export function updateRunStatus(
  ...
  const completedAt = ["completed", "aborted", "failed"].includes(status)
    ? "datetime('now')"
    : "NULL";
...
export function getLatestRun(db: Database, planPath: string): RunRow | null {
  return db
    .query("SELECT * FROM runs WHERE plan_path = ?1 ORDER BY rowid DESC LIMIT 1")
    .get(planPath) as RunRow | null;
}
...
export function getAgentResults(
  ...
  return db
    .query("SELECT * FROM agent_results WHERE run_id = ?1 ORDER BY phase ASC, iteration ASC")

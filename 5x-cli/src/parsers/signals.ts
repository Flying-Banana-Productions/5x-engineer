...
export function parseVerdictBlock(text: string): VerdictBlock | null {
	...
	try {
		...
		const { protocolVersion, readiness, reviewPath, items } = parsed as Record<
			string,
			unknown
		>;
...
export function parseStatusBlock(text: string): StatusBlock | null {
	...
	try {
		...
		if (data.protocolVersion !== undefined && data.protocolVersion !== 1) {
			console.warn(
				`[5x] Unknown status protocolVersion: ${data.protocolVersion}`,
			);
		}
		...
		if (
			...
			!validResults.includes(data.result)
		) {
			return null;
		}

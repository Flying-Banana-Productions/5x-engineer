...
describe("extractPipeContext", () => {
	test("extracts run_id, step_name, phase from data", () => {
	...
	test("builds templateVars from string fields only", () => {
		const ctx = extractPipeContext({
	...
	test("skips excluded metadata keys", () => {
		const ctx = extractPipeContext({
			run_id: "run_abc",
			session_id: "sess_xyz",
	...
	test("skips values with newlines", () => {
		const ctx = extractPipeContext({
	...
	test("skips non-string values (objects, arrays, numbers, booleans)", () => {
		const ctx = extractPipeContext({
			obj: { nested: true },
			arr: [1, 2, 3],
			num: 42,
	...
	test("returns empty context for empty data", () => {
		const ctx = extractPipeContext({});
	...
	test("ignores non-string run_id, step_name, phase", () => {
		const ctx = extractPipeContext({
			run_id: 123,
			step_name: true,
			phase: null,
...
describe("extractInvokeMetadata", () => {
	test("detects invoke shape and extracts all fields", () => {
		const data = {
			run_id: "run_abc123",
			step_name: "author:implement",
			phase: "1",
	...
	test("returns null for non-invoke data (quality output)", () => {
		const data = {
			passed: true,
			results: [
	...
	test("returns null when result is a string (not an object)", () => {
		const data = {
			result: "some string",
			session_id: "sess_xyz",
		};
	...
	test("returns null when result is an array", () => {
		const data = {
			result: [1, 2, 3],
			session_id: "sess_xyz",
		};
	...
	test("handles cost_usd: null (not a number)", () => {
		const data = {
			result: { result: "complete" },
			session_id: "sess_xyz",
			cost_usd: null,
...
describe("readUpstreamEnvelope", () => {
	...
	const HELPER_PATH = join(import.meta.dir, "helpers", "pipe-read-helper.ts");
	...
	test("parses valid invoke envelope", async () => {
	...
	test("parses valid non-invoke envelope (quality)", async () => {
		const envelope = JSON.stringify({
			ok: true,
			data: {
				passed: true,
	...
	test("throws on error envelope (ok: false)", async () => {
		const envelope = JSON.stringify({
			ok: false,
	...
	test("throws when envelope missing ok field", async () => {

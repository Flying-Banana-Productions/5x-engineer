...
describe("run-context resolver: fail-closed worktree contract", () => {
	test(
		"mapped worktree missing returns WORKTREE_MISSING for state reads",
		() => {
	...
	test(
		"mapped worktree present succeeds",
		() => {
			const planPath = join(tmp, "docs", "plan.md");
			mkdirSync(join(tmp, "docs"), { recursive: true });
			writeFileSync(planPath, "# Plan\n");
	...
	test(
		"completed run with missing worktree still fails WORKTREE_MISSING",
		() => {
			const planPath = join(tmp, "docs", "plan.md");
			mkdirSync(join(tmp, "docs"), { recursive: true });
			writeFileSync(planPath, "# Plan\n");
	...
	test(
		"plan path outside controlPlaneRoot returns PLAN_PATH_INVALID",
		() => {
...
describe("run state --run with worktree context", () => {
	test(
		"run state reports worktree_path when run has mapping",
		async () => {
	...
	test(
		"run state fails with WORKTREE_MISSING when worktree is gone",
		async () => {
			const dir = makeTmpDir();
			try {
				const projectRoot = setupProject(dir);
...
describe("run complete --run with worktree context", () => {
	test(
		"run complete fails with WORKTREE_MISSING when worktree is gone",
		async () => {
			const dir = makeTmpDir();
			try {
				const projectRoot = setupProject(dir);
...
describe("diff --run error paths", () => {
	test(
		"diff --run fails with RUN_NOT_FOUND for unknown run",
		async () => {
			const dir = makeTmpDir();
			try {
				const projectRoot = setupProject(dir);
	...
	test(
		"diff --run fails with WORKTREE_MISSING when worktree is gone",
		async () => {
			const dir = makeTmpDir();
			try {
				const projectRoot = setupProject(dir);
...
describe("diff without --run backward compat", () => {
	test(
		"diff without --run remains unchanged (produces diff output)",
		async () => {
			const dir = makeTmpDir();
			try {
				const projectRoot = setupProject(dir);
...
describe("quality run --run error paths", () => {
	test(
		"quality run --run fails with RUN_NOT_FOUND for unknown run",
		async () => {
			const dir = makeTmpDir();
			try {
				const projectRoot = setupProject(dir);
	...
	test(
		"quality run --run fails with WORKTREE_MISSING when worktree is gone",
		async () => {
			const dir = makeTmpDir();
			try {
				const projectRoot = setupProject(dir);
...
describe("run init plan-path validation", () => {
	test(
		"run init rejects plan outside controlPlaneRoot",
		async () => {
			const dir = makeTmpDir();
			try {
				const projectRoot = setupProject(dir);
	...
	test(
		"run init accepts plan inside controlPlaneRoot",
		async () => {
			const dir = makeTmpDir();
			try {
				const projectRoot = setupProject(dir);

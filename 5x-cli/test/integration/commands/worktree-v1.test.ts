...
describe("5x worktree", () => {
	test(
		"create: creates a new worktree and returns JSON",
		async () => {
	...
	test(
		"create: idempotent — returns existing worktree with created=false",
		async () => {
			const dir = makeTmpDir();
			try {
				const { planPath } = setupProject(dir);
	...
	test(
		"create: accepts custom --branch name",
		async () => {
			const dir = makeTmpDir();
			try {
				const { planPath } = setupProject(dir);
	...
	test(
		"attach: maps an existing git worktree path to a plan",
		async () => {
			const dir = makeTmpDir();
			try {
				const { planPath } = setupProject(dir);
	...
	test(
		"attach: returns WORKTREE_INVALID for non-worktree path",
		async () => {
			const dir = makeTmpDir();
			try {
				const { planPath } = setupProject(dir);
	...
	test(
		"remove: handles missing directory gracefully",
		async () => {
			const dir = makeTmpDir();
			try {
				const { planPath } = setupProject(dir);
	...
	test(
		"list: returns empty array when no worktrees",
		async () => {
			const dir = makeTmpDir();
			try {
				setupProject(dir);
	...
	test(
		"list: returns worktrees after creation",
		async () => {
			const dir = makeTmpDir();
			try {
				const { planPath } = setupProject(dir);
				...
				expect(payload.worktrees[0]?.worktree_path).toContain(
					"001-test-feature",
				);
	...
	test(
		"create: returns PLAN_NOT_FOUND for missing plan file",
		async () => {
			const dir = makeTmpDir();
			try {
				setupProject(dir);
	...
	test(
		"create: postCreate hook output does not contaminate stdout JSON",
		async () => {
			const dir = makeTmpDir();
			try {
				const { planPath } = setupProject(dir);
	...
	test(
		"create: failing postCreate hook is non-fatal with warnings in response",
		async () => {
			const dir = makeTmpDir();
			try {
				const { planPath } = setupProject(dir);
	...
	test(
		"full lifecycle: create → list → remove → list empty",
		async () => {
			const dir = makeTmpDir();
			try {
				const { planPath } = setupProject(dir);

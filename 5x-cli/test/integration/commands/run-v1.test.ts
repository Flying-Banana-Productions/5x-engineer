...
describe("5x run lifecycle", () => {
	...
	test(
		"numeric args validation — NaN and negative values rejected",
		async () => {
			const dir = makeTmpDir();
			try {
				const { planPath, projectRoot } = setupProject(dir);
				...
				expect(
					(parseJson(tokensResult.stdout).error as Record<string, unknown>)
						.code,
				).toBe("INVALID_ARGS");
	...
	test(
		"numeric args reject trailing junk (strict parsing)",
		async () => {
			const dir = makeTmpDir();
			try {
				const { planPath, projectRoot } = setupProject(dir);
				...
				expect(
					(parseJson(durationJunk.stdout).error as Record<string, unknown>)
						.code,
				).toBe("INVALID_ARGS");
			} finally {
				cleanupDir(dir);

...
describe("run record pipe ingestion", () => {
	test("pipe invoke envelope → record auto-extracts all fields", async () => {
	...
	test("pipe quality envelope → record uses JSON.stringify(data) as result, requires explicit step name and --run", async () => {
		const dir = makeTmpDir();
		try {
			const { projectRoot, runId } = await setupProjectWithRun(dir);
			...
			const steps = (parseJson(state.stdout).data as Record<string, unknown>)
				.steps as Array<Record<string, unknown>>;
			const resultJson = JSON.parse(steps[0]?.result_json as string) as Record<
				string,
				unknown
			>;
	...
	test("CLI flags override piped values (e.g., --phase 2 overrides piped phase: '1')", async () => {
		const dir = makeTmpDir();
		try {
			const { projectRoot, runId } = await setupProjectWithRun(dir);
	...
	test("positional step name overrides piped step_name", async () => {
		const dir = makeTmpDir();
		try {
			const { projectRoot, runId } = await setupProjectWithRun(dir);
	...
	test("error when stdin not piped and required params missing", async () => {
		const dir = makeTmpDir();
		try {
			...
			expect((data.error as Record<string, unknown>).code).toBe("INVALID_ARGS");
		} finally {
			cleanupDir(dir);
		}
	});

	test("error when piped envelope is ok: false", async () => {
		const dir = makeTmpDir();
		try {
			const { projectRoot } = await setupProjectWithRun(dir);
	...
	test("--result @./file.json with piped envelope → result from file, run_id/step_name from pipe", async () => {
		const dir = makeTmpDir();
		try {
			const { projectRoot, runId } = await setupProjectWithRun(dir);
	...
	test("--result=- consumes stdin for raw result (not envelope parsing)", async () => {
		const dir = makeTmpDir();
		try {
			const { projectRoot, runId } = await setupProjectWithRun(dir);
	...
	test('--result \'{"inline":"json"}\' with piped envelope → result from inline, context from pipe', async () => {
		const dir = makeTmpDir();
		try {
			const { projectRoot, runId } = await setupProjectWithRun(dir);
	...
	test("pipe with explicit --run (partial override — run from CLI, result from pipe)", async () => {
		const dir = makeTmpDir();
		try {
			const { projectRoot, runId } = await setupProjectWithRun(dir);

...
describe("resolveControlPlaneRoot", () => {
	test("from root checkout: returns root path, managed mode", () => {
		initRepo(tmp);
		createStateDb(tmp);
	...
	test("from nested linked worktree: resolves to root state DB", () => {
		initRepo(tmp);
		createStateDb(tmp);
	...
	test("from externally attached worktree: resolves to root state DB via git common-dir", () => {
		initRepo(tmp);
		createStateDb(tmp);
	...
	test("worktree with local state DB but root DB also exists: managed mode wins", () => {
		initRepo(tmp);
		createStateDb(tmp);
	...
	test("worktree with local state DB and no root DB: isolated mode", () => {
		initRepo(tmp);
	...
	test("no git context: returns mode 'none'", () => {
		// tmp has no .git
		const result = resolveControlPlaneRoot(tmp);
	...
	test("git repo with no state DB anywhere: returns mode 'none'", () => {
		initRepo(tmp);
		// No state DB created
	...
	test("custom db.path directory (non-default)", () => {
		initRepo(tmp);
		writeConfig(tmp, `[db]\npath = "custom-state"\n`);
	...
	test("absolute db.path: resolver finds DB at absolute path", () => {
		initRepo(tmp);
	...
	test("legacy file-style db.path normalized (e.g. '.5x/5x.db' → '.5x')", () => {
		initRepo(tmp);
		writeConfig(tmp, `[db]\npath = ".5x/5x.db"\n`);
	...
	test("relative git-common-dir resolved correctly relative to git-dir", () => {
	...
	test("isolated mode: controlPlaneRoot is the checkout root, not main repo root", () => {
		initRepo(tmp);
	...
	test("isolated mode: db.path read from checkout root config, not main repo config", () => {
		initRepo(tmp);

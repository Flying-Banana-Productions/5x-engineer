...
describe("5x diff", () => {
	...
	test("returns diff of working tree changes against HEAD", async () => {
		const dir = makeTmpDir();
		try {
			setupProject(dir);
	...
	test("returns diff since a specific ref", async () => {
		const dir = makeTmpDir();
		try {
			setupProject(dir);
	...
	test("includes stat when --stat is specified", async () => {
		const dir = makeTmpDir();
		try {
			setupProject(dir);
	...
	test("stat not included by default", async () => {
		const dir = makeTmpDir();
		try {
			setupProject(dir);
	...
	test("returns INVALID_REF for nonexistent ref", async () => {
		const dir = makeTmpDir();
		try {
			setupProject(dir);
	...
	test("handles --since with --stat combined", async () => {
		const dir = makeTmpDir();
		try {
			setupProject(dir);

...
describe("5x run watch", () => {
	test("rejects invalid run-id", async () => {
		...
		try {
			...
			expect((json.error as Record<string, unknown>).code).toBe("INVALID_ARGS");
		} finally {
			rmSync(dir, { recursive: true, force: true });
		}
	});

	test("errors with RUN_NOT_FOUND when neither DB entry nor log dir exists", async () => {
		const dir = makeTmpDir();
		try {
			const projectRoot = setupProject(dir);
			const result = await run5x(projectRoot, [
	...
	test("warns and proceeds when DB entry missing but log dir exists", async () => {
		const dir = makeTmpDir();
		try {
			const projectRoot = setupProject(dir);
	...
	test("outputs raw NDJSON by default with source field", async () => {
		const dir = makeTmpDir();
		try {
			const projectRoot = setupProject(dir);
	...
	test("--human-readable renders session_start as label header", async () => {
		const dir = makeTmpDir();
		try {
			const projectRoot = setupProject(dir);
			const runId = await initRun(projectRoot);
	...
	test("--human-readable handles multi-file with label switching", async () => {
		const dir = makeTmpDir();
		try {
			const projectRoot = setupProject(dir);
			const runId = await initRun(projectRoot);
	...
	test("--no-replay skips existing content", async () => {
		const dir = makeTmpDir();
		try {
			const projectRoot = setupProject(dir);
			const runId = await initRun(projectRoot);
	...
	test("--human-readable falls back to filename for unknown files", async () => {
		const dir = makeTmpDir();
		try {
			const projectRoot = setupProject(dir);
			const runId = await initRun(projectRoot);

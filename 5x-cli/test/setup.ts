...
// Canonicalize TMPDIR so macOS `/var` vs `/private/var` does not make
// `path.resolve` and `realpath` disagree in tests that compare stored paths.
try {
	process.env.TMPDIR = realpathSync(tmpdir());
} catch {
	// Keep the platform default if realpath fails.
}
